...
3. The compaction event replaces the originals in the session

If `llm` is provided, the summary is generated via an LLM call. Otherwise, a simple text extraction fallback is used.

### Safety

Compaction never runs mid-stream — only after all agent events have been yielded. Events with unresolved tool calls are never compacted.
...
`LlmAgent` automatically detects compaction events and includes the summary in the LLM context via `apply_compaction()` from `orxhestra.events.filters`.
